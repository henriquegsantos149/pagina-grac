...
export default function SkillsResults() {
  return (
    <section className="py-16 md:py-24 bg-[var(--color-brand-light)] text-[var(--color-brand-dark)]">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-[var(--color-brand-dark)]/70 max-w-2xl mx-auto">Desenvolva as competências mais valorizadas pelo mercado de remediação de áreas contaminadas.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {skills.map((skill, index) => (
            ...
            >
              <div className="w-12 h-12 shape-leaf bg-white/5 flex items-center justify-center mb-6 group-hover:bg-brand-gradient group-hover:text-[var(--color-brand-light)] transition-all">
                <skill.icon className="w-6 h-6 text-[var(--color-brand-primary)] group-hover:text-[var(--color-brand-light)]" />
              </div>
              <h3 className="text-xl font-bold mb-3 font-primary uppercase tracking-wide text-[var(--color-brand-light)] group-hover:text-[var(--color-brand-primary)] transition-colors">
                {skill.title}
              </h3>
